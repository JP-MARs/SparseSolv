...
/*//=======================================================
// ● Eigen BiCGstab_accelで解く
//=======================================================*/
bool MatSolvers::solveEigenBiCGstab_accel_py(const slv_int size0, const double conv_cri, const int max_ite, const SparseMat& matA, py::array_t<double> vecB, py::array_t<double> results, bool init){
	/* Python側へのポインタ設定 */
	auto b_buf = vecB.request();
    double *b_ptr = (double *)b_buf.ptr;
	auto x_buf = results.request();
	...
	bool bl = MatSolversEigenMKL::solveEigenBiCGstab_accel(size0, conv_cri, max_ite, matA, vecBa, results_a, init);
	for(int i = 0 ; i < size0 ; i++){
		x_ptr[i] = results_a[i];
	}
	delete[] vecBa;
...
bool MatSolvers::solveEigenBiCGstab_accel_py(const slv_int size0, const double conv_cri, const int max_ite,const SparseMatC& matA, py::array_t<std::complex<double>> vecB, py::array_t<std::complex<double>> results, bool init){
	/* Python側へのポインタ設定 */
	auto b_buf = vecB.request();
    std::complex<double> *b_ptr = (std::complex<double> *)b_buf.ptr;
	auto x_buf = results.request();
	...
	std::complex<double>* vecBa = new std::complex<double>[size0];
	std::complex<double>* results_a = new std::complex<double>[size0];
	for(int i = 0 ; i < size0 ; i++){
		const std::complex<double> temp = b_ptr[i];
		vecBa[i] = temp;
		results_a[i] = x_ptr[i];
	}
	bool bl = MatSolversEigenMKL::solveEigenBiCGstab_accel(size0, conv_cri, max_ite, matA, vecBa, results_a, init);
...
// ● Eigen GMRESで解く
//=======================================================*/
bool MatSolvers::solveEigenGMRES_py(const slv_int size0, const double conv_cri, const int max_ite, const SparseMat& matA, py::array_t<double> vecB, py::array_t<double> results, bool init){
	/* Python側へのポインタ設定 */
	auto b_buf = vecB.request();
    double *b_ptr = (double *)b_buf.ptr;
	auto x_buf = results.request();
	...
	bool bl = MatSolversEigenMKL::solveEigenGMRES(size0, conv_cri, max_ite, matA, vecBa, results_a, init);
	for(int i = 0 ; i < size0 ; i++){
		x_ptr[i] = results_a[i];
	}
	delete[] vecBa;
...
bool MatSolvers::solveEigenGMRES_py(const slv_int size0, const double conv_cri, const int max_ite,const SparseMatC& matA, py::array_t<std::complex<double>> vecB, py::array_t<std::complex<double>> results, bool init){
	/* Python側へのポインタ設定 */
	auto b_buf = vecB.request();
    std::complex<double> *b_ptr = (std::complex<double> *)b_buf.ptr;
	auto x_buf = results.request();
