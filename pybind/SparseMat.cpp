/**
* @file SparseMat.cpp
* @brief sparse matrix class source file
*/
...
SparseMat SparseMat::inv() const{
	...
	Eigen::MatrixXd temp_mat = matrix.matrix.toDense();
	/* 逆行列 */
	temp_mat = temp_mat.inverse();
	/* 確定 */
	SparseMatBaseD matrix_new(the_size);
	for(slv_int i = 0 ; i < the_size ; i++){
		for(slv_int j = 0 ; j < the_size ; j++){
			double value = temp_mat(i, j);
	...
	matrix_new.fix();
	/* 結果を本体行列にムーブし、終わる */
	SparseMat mat_ans(std::move(matrix_new));
	return mat_ans;
	/*
	SparseMatBaseD mat_ori;
