...
/*//=======================================================
  // ● 全体係数マトリックス作成
  //=======================================================*/
void VoxelMesh::make_matrix(JPMRspace::SparseSolv::SparseMat& matA){
	const int size = edge_num - A0_size;
	...
	JPMRspace::SparseSolv::SparseBuilder builder(size);
	...
	for(int i = 0 ; i < element_num ; i++){
		...
		for(int k = 0 ; k < 12 ; k++){
			...
			for(int m = k ; m < 12 ; m++){
				...
				/* 代入～！ */
				builder.add(k_sub, m_sub, temp);
				if(k != m){
					builder.add(m_sub, k_sub, temp);
				}
			}
		}
		Ele_ptr++;
