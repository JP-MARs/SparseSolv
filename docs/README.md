...
*   **複素対称行列への拡張（ICCOCG）**:
    *   日本国内では、Van der Vorst の1990年の論文 **[4]** が発表される以前から、複素対称行列に対する ICCG法の拡張（ICCOCG）が実用化されていました。渦電流解析などの周波数応答解析において、複素数に対しても Shifted IC を適用し「特異行列のまま解く」スタイルが既に確立されていたのです。
