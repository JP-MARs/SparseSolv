/**
* @file SparseMatC.cpp
* @brief sparse matrix (complex) class source file
*/
...
// ● 行列の完全初期化
//=======================================================*/
void SparseMatC::all_initialize(slv_int x){
	matrix.is_fix = false;
...
SparseMatC SparseMatC::inv() const{
	const slv_int the_size = matrix.size;
	Eigen::MatrixXcd temp_mat = matrix.matrix.toDense();
	/* 逆行列 */
	temp_mat = temp_mat.inverse();
	/* 確定 */
	SparseMatBaseC matrix_new(the_size);
	for(slv_int i = 0 ; i < the_size ; i++){
		for(slv_int j = 0 ; j < the_size ; j++){
			dcomplex value = temp_mat(i, j);
			double abs_val = abs(value);
			if(abs_val > 1.0e-12){
				matrix_new.add(i, j, value);
			}
		}
	}
	matrix_new.fix();
	/* 結果を本体行列にムーブし、終わる */
	SparseMatC mat_ans(std::move(matrix_new));
	return mat_ans;
...
//=======================================================*/
/** 
 * @brief IC decomposition with accelaration factor
 * @param diagD diagonal matrix D (size: size0)
 * @param accela accelaration factor
*/
