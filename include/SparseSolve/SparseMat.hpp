/**
 * @file SparseMat
 * @brief Sparse Matrix for double precision
 *
 * このファイルはdouble用スパース行列クラス SparseMat を定義しています。
 * 内部でSparseMatBaseDを保持し、行列の生成・操作・演算を行います。行列の実態はSparseMatBaseDが管理します。
 */
...
/**
 * @class SparseMat
 * @brief Main Sparse matrix (double precision)
 * このファイルは本ライブラリが提供する実数疎行列 SparseMat を定義します。
 * 内部でSparseMatBaseDを保持し、行列の生成・操作・演算を行います。行列の実態はSparseMatBaseDが管理します。
 * このクラスはSparseMatBaseDに対する演算等を定義しています。
 */
class SparseMat{	
