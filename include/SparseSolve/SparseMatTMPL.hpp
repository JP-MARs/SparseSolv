 ...
 * 必要に応じて Eigen::SparseMatrix に確定（fix）します。
 * 行列構成時は一時保存状態で利用することが一般的です。
 * 行列が完成した場合、fix() メソッドを呼び出して確定させます。
 * その後は Eigen の高速な演算が利用可能になります。
 */
...
/**
* @typedef slv_int
* @brief Aliaas for integer type used in solvers
*/
using slv_int = int;// int;
...
/**
 * @class SparseMatTMPL
 * @brief Sparse matrix template (this is subclass used in main matrix class)
 * このファイルはテンプレートクラス SparseMatTMPL を提供します。
 * このクラスは SparseMat, SparseMatC の内部で利用されます。
 * 内部では一時保存用の map[] を用いて要素を構築し、
 * 必要に応じて Eigen::SparseMatrix に確定（fix）します。
 * 行列構成時は一時保存状態で利用することが一般的です。
 * 行列が完成した場合、fix() メソッドを呼び出して確定させます。
...
private:
	/** @brief row size */
	slv_int size;																/* 行列の行数 */
	/** @brief fixed flag (true: Eigen matrix is constructed) */
	...
	/*-------------------------------------------------------*/
	/** @brief Add value to fixed matrix */
...
/** 
 * @brief Constructor with row size
...
/* 作成済み疎行列データから初期化 */
/** 
 * @brief Constructor with non-zero data
*/
template<typename DType>
...
/** 
 * @brief Constructor(real) with non-zero data
*/
template<typename DType>
...
//=======================================================*/
/** 
 * @brief initialize temporary matrix with size
 * @param ss row size
*/
template<typename DType>
...
//=======================================================*/
/** 
 * @brief re-initialize temporary matrix with size
 * @param new_size row size
*/
template<typename DType>
...
//=======================================================*/
/** 
 * @brief make empty square sparse matrix (fixed)
 * @param size0 row size
*/
template<typename DType>
...
//=======================================================*/
/** 
 * @brief fix temporary matrix to sparse matrix
 * @param toSquare true: make square matrix by adding zeros
*/
template<typename DType>
...
bool SparseMatTMPL<DType>::isEmpty() const{
	const auto nun_zero = matrix.nonZeros();
	return( nun_zero == 0 );
}
...
//=======================================================*/
/** 
 * @brief add value to temporary matrix
 * @param gyo row index
 * @param retu column index
